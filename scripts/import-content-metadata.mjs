...
if (!(organizationId && inputPath)) {
  console.error(
    'Usage: node scripts/import-content-metadata.mjs <orgId> <inputPath>'
  )
  process.exit(1)
}
...
const nextContentTypes = Array.isArray(data.contentTypes)
  ? data.contentTypes
  : []
const nextCustomFields = Array.isArray(data.customFields)
  ? data.customFields
  : []
const nextContentEntries =
  typeof data.contentEntries === 'object' && data.contentEntries
    ? data.contentEntries
    : {}
const nextSnapshots = Array.isArray(data.contentSnapshots)
  ? data.contentSnapshots
  : []
